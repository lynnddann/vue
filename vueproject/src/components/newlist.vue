...
export default {
  ...
  mounted(){
      ...
      body:`time=Y&pageindex=${this.current}&pagesize=10`
  ...
  methods:{
  	handleScroll(){ //出现回到顶部按钮
  		//console.log(document.getElementsByTagName("ul")[0].offsetTop) 
  		//console.log(this.$store.state.offsettop)		
  		var scrolltop = document.documentElement.scrollTop || document.body.scrollTop
  		//var offsettop = document.querySelector(".navlist ul").offsetTop;
  		if(scrolltop>400){
  			this.isshow = true
  		}else{
  			this.isshow = false
